...
TSharedRef<FVoxelDragConnection> FVoxelDragConnection::New(const TSharedRef<SGraphPanel>& GraphPanel, const FDraggedPinTable& DraggedPins)
{
	TSharedRef<FVoxelDragConnection> Operation = MakeShareable(new FVoxelDragConnection(GraphPanel, DraggedPins));
	Operation->Construct();

	return Operation;
}

void FVoxelDragConnection::OnDrop( bool bDropWasHandled, const FPointerEvent& MouseEvent )
{
...
void FVoxelDragConnection::OnDragged(const class FDragDropEvent& DragDropEvent)
{
...
void FVoxelDragConnection::HoverTargetChanged()
{
...
FVoxelDragConnection::FVoxelDragConnection(const TSharedRef<SGraphPanel>& GraphPanelIn, const FDraggedPinTable& DraggedPinsIn)
...
FReply FVoxelDragConnection::DroppedOnPin(FVector2D ScreenPosition, FVector2D GraphPosition)
{
	TArray<UEdGraphPin*> ValidSourcePins;
	ValidateGraphPinList(/*out*/ ValidSourcePins);
...
FReply FVoxelDragConnection::DroppedOnNode(FVector2D ScreenPosition, FVector2D GraphPosition)
{
...
#else
					FName PinName = SourcePin->PinFriendlyName.IsEmpty()? SourcePin->PinName : *SourcePin->PinFriendlyName.ToString();
#endif
...
FReply FVoxelDragConnection::DroppedOnPanel( const TSharedRef< SWidget >& Panel, FVector2D ScreenPosition, FVector2D GraphPosition, UEdGraph& Graph)
{
	// Gather any source drag pins
...
void FVoxelDragConnection::ValidateGraphPinList(TArray<UEdGraphPin*>& OutValidPins)
{
