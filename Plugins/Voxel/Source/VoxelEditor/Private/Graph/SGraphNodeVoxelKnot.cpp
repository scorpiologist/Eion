...
class FAmbivalentDirectionDragConnection : public FVoxelDragConnection
{
public:
...
protected:
	FAmbivalentDirectionDragConnection(UEdGraphNode* InKnot, const TSharedRef<SGraphPanel>& InGraphPanel, const FDraggedPinTable& InStartingPins)
		: FVoxelDragConnection(InGraphPanel, InStartingPins)
		, KnotPtr(InKnot)
...
void FAmbivalentDirectionDragConnection::OnDragged(const class FDragDropEvent& DragDropEvent)
{
	...
	FVoxelDragConnection::OnDragged(DragDropEvent);
}
...
void FAmbivalentDirectionDragConnection::ValidateGraphPinList(TArray<UEdGraphPin*>& OutValidPins)
{
   ...
   if (KnotPtr.Get() != nullptr)
   {
	...
	else
	{
		...
		FVoxelDragConnection::ValidateGraphPinList(OutValidPins);
	}
}
...
void SGraphPinKnot::OnDragEnter(const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent)
{
	...
	if (Operation.IsValid() && Operation->IsOfType<FVoxelDragConnection>())
	{
		TSharedPtr<FVoxelDragConnection> DragConnectionOp = StaticCastSharedPtr<FVoxelDragConnection>(Operation);
...
void SGraphNodeVoxelKnot::Construct(const FArguments& InArgs, UEdGraphNode* InKnot)
{
	int32 InputPinIndex = -1;
	int32 OutputPinIndex = -1;
	verify(InKnot->ShouldDrawNodeAsControlPointOnly(InputPinIndex, OutputPinIndex) == true &&
		InputPinIndex >= 0 && OutputPinIndex >= 0);
	SGraphNodeDefaultVoxel::Construct(SGraphNodeDefaultVoxel::FArguments().GraphNodeObj(InKnot));
}


void SGraphNodeVoxelKnot::UpdateGraphNode()
{
	InputPins.Empty();
	OutputPins.Empty();
	...
	InlineEditableText = SNew(SInlineEditableTextBlock)
		...
		.Text(this, &SGraphNodeVoxelKnot::GetEditableNodeTitleAsText)
		.OnVerifyTextChanged(this, &SGraphNodeVoxelKnot::OnVerifyNameTextChanged)
		.OnTextCommitted(this, &SGraphNodeVoxelKnot::OnNameTextCommited)
		.IsReadOnly(this, &SGraphNodeVoxelKnot::IsNameReadOnly)
		.IsSelected(this, &SGraphNodeVoxelKnot::IsSelectedExclusively);
...
// 			.HAlign(HAlign_Center)
	...
	.SlotOffset(TAttribute<FVector2D>(this, &SGraphNodeVoxelKnot::GetCommentOffset))
...
const FSlateBrush* SGraphNodeVoxelKnot::GetShadowBrush(bool bSelected) const
{
	return bSelected ? FEditorStyle::GetBrush(TEXT("Graph.Node.ShadowSelected")) : FEditorStyle::GetNoBrush();
}

TSharedPtr<SGraphPin> SGraphNodeVoxelKnot::CreatePinWidget(UEdGraphPin* Pin) const
{
	return SNew(SGraphPinKnot, Pin);
}

void SGraphNodeVoxelKnot::AddPin(const TSharedRef<SGraphPin>& PinToAdd)
{
...
FVector2D SGraphNodeVoxelKnot::GetCommentOffset() const
{
...
void SGraphNodeVoxelKnot::OnCommentBubbleToggled(bool bInCommentBubbleVisible)
{
	SGraphNode::OnCommentBubbleToggled(bInCommentBubbleVisible);
	bAlwaysShowCommentBubble = bInCommentBubbleVisible;
}

void SGraphNodeVoxelKnot::OnCommentTextCommitted(const FText& NewComment, ETextCommit::Type CommitInfo)
{
...
void SGraphNodeVoxelKnot::OnMouseEnter(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
...
void SGraphNodeVoxelKnot::OnMouseLeave(const FPointerEvent& MouseEvent)
{
