...
void FVoxelData::AddAsset(TSharedRef<FVoxelAssetInstance> Asset)
{
	auto Octrees = BeginSet(FIntBox::Infinite());
	MainOctree->AddAsset(Asset);
	EndSet(Octrees);
}
...
void FVoxelData::GetDiffQueues(TArray<FVoxelValueDiff>& OutValueDiffQueue, TArray<FVoxelMaterialDiff>& OutMaterialDiffQueue)
{
	auto Octrees = BeginGet(FIntBox::Infinite());

	MainOctree->AddChunksToDiffQueues(OutValueDiffQueue, OutMaterialDiffQueue);

	EndGet(Octrees);
}

void FVoxelData::LoadFromDiffQueuesAndGetModifiedPositions(TArray<FVoxelValueDiff>& ValueDiffQueue, TArray<FVoxelMaterialDiff>& MaterialDiffQueue, TArray<FIntVector>& OutModifiedPositions)
{
	// Values
	{
		TArray<FValueOctree*> OctreesToModify;
		{
			TArray<uint64> Ids;
			{
				Ids.SetNumUninitialized(ValueDiffQueue.Num());
				for (int Index = 0; Index < Ids.Num(); Index++)
				{
					// ValueDiffQueue is reversed
					Ids[Index] = ValueDiffQueue[ValueDiffQueue.Num() - 1 - Index].Id;
				}
			}

			auto Octrees = BeginGet(FIntBox::Infinite());
			// OctreesToModify is reversed, but ValueDiffQueue too, so it's not an issue
			MainOctree->GetLeavesWithIds(Ids, OctreesToModify);
			check(Ids.Num() == 0);
			EndGet(Octrees);
		}

		{
			check(ValueDiffQueue.Num() == OctreesToModify.Num());
			int Index = 0;
			while (Index < OctreesToModify.Num())
			{
				auto& Octree = OctreesToModify[Index];

				auto Octrees = BeginSet(Octree->GetBounds());
				while (Index < OctreesToModify.Num() && Octree == OctreesToModify[Index])
				{
					Octree->LoadValueDiff(ValueDiffQueue[Index], OutModifiedPositions);
					Index++;
				}
				EndSet(Octrees);
			}
	...
	// Materials
	{
		TArray<FValueOctree*> OctreesToModify;
		{
			TArray<uint64> Ids;
			{
				Ids.SetNumUninitialized(MaterialDiffQueue.Num());
				for (int Index = 0; Index < Ids.Num(); Index++)
				{
					// MaterialDiffQueue is reversed
					Ids[Index] = MaterialDiffQueue[MaterialDiffQueue.Num() - 1 - Index].Id;
				}
			}

			auto Octrees = BeginGet(FIntBox::Infinite());
			// OctreesToModify is reversed, but MaterialDiffQueue too, so it's not an issue
			MainOctree->GetLeavesWithIds(Ids, OctreesToModify);
			check(Ids.Num() == 0);
			EndGet(Octrees);
		}
		...
		{
			check(MaterialDiffQueue.Num() == OctreesToModify.Num());
			int Index = 0;
			while (Index < OctreesToModify.Num())
			{
				auto& Octree = OctreesToModify[Index];
				...
				while (Index < OctreesToModify.Num() && Octree == OctreesToModify[Index])
				{
					Octree->LoadMaterialDiff(MaterialDiffQueue[Index], OutModifiedPositions);
					Index++;
				}
				EndSet(Octrees);
			}
