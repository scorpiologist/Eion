...
bool FValueOctree::IsEmpty(const FIntVector& Start, const int Step, const FIntVector& Size) const
{
	...
	{
		if (Asset->GetWorldBounds().Intersect(InBounds) && !Asset->IsEmpty(Start, Step, Size))
		{
			return false;
		}
	}
...
void FValueOctree::GetValuesAndMaterials(float InValues[], FVoxelMaterial InMaterials[], const FIntVector& Start, const FIntVector& StartIndex, const int Step, const FIntVector& Size, const FIntVector& ArraySize) const
{
	...
	if (IsLeaf())
	{
		...
		{
			float* AssetValues = InValues ? new float[Size.X * Size.Y * Size.Z] : nullptr;
			FVoxelMaterial* AssetMaterials = InMaterials ? new FVoxelMaterial[Size.X * Size.Y * Size.Z] : nullptr;
			FVoxelType* AssetVoxelTypes = new FVoxelType[Size.X * Size.Y * Size.Z];
			Assets[0]->GetValuesAndMaterialsAndVoxelTypes(AssetValues, AssetMaterials, AssetVoxelTypes, Start, FIntVector::ZeroValue, Step, Size, Size);
			...
			for (int K = 0; K < Size.Z; K++)
			{
				...
				for (int J = 0; J < Size.Y; J++)
				{
					...
					for (int I = 0; I < Size.X; I++)
					{
						...
						const int GlobalIndex = (StartIndex.X + I) + ArraySize.X * (StartIndex.Y + J) + ArraySize.X * ArraySize.Y * (StartIndex.Z + K);
						const int AssetIndex = I + Size.X * J + Size.X * Size.Y * K;

						check(0 <= AssetIndex && AssetIndex < Size.X * Size.Y * Size.Z);
						FVoxelType VoxelType = AssetVoxelTypes[AssetIndex];
						if (InMaterials)
						{
							FVoxelMaterial Material;

							EVoxelMaterialType MaterialType = VoxelType.GetMaterialType();
							if (MaterialType == EVoxelMaterialType::UseMaterial)
							{
								Material = AssetMaterials[AssetIndex];
							}
							else
							{
								FVoxelAssetInstance* CurrentAsset = nullptr;
								int i = 1;
								while (i < Assets.Num())
								{
									CurrentAsset = &(Assets[i].Get());
									MaterialType = CurrentAsset->GetVoxelType(X, Y, Z).GetMaterialType();

									if (MaterialType == EVoxelMaterialType::UseMaterial)
									{
										Material = CurrentAsset->GetMaterial(X, Y, Z);
										break;
									}
									i++;
								}
								if (MaterialType != EVoxelMaterialType::UseMaterial)
								{
									Material = WorldGenerator->GetMaterial(X, Y, Z);
								}
							}
							InMaterials[GlobalIndex] = Material;
						}
						...
						if (InValues)
						{
							...
							EVoxelValueType ValueType = VoxelType.GetValueType();
							if (ValueType == EVoxelValueType::UseValue)
							{
								Value = AssetValues[AssetIndex];
							}
							else
							{
								struct FTmpValueTypeValue
								{
									EVoxelValueType Type;
									float Value;
									FTmpValueTypeValue(EVoxelValueType T, float V) : Type(T), Value(V) {}
								};

								TArray<FTmpValueTypeValue> Stack;
								Stack.Add(FTmpValueTypeValue(ValueType, AssetValues[AssetIndex]));
								...
								while (i < Assets.Num())
								{
									...
									FVoxelType CurrentVoxelType;
									FVoxelMaterial CurrentMaterial;
									float CurrentValue;
									CurrentAsset->GetValueAndMaterialAndVoxelType(X, Y, Z, CurrentValue, CurrentMaterial, CurrentVoxelType);

									ValueType = CurrentVoxelType.GetValueType();

									Stack.Add(FTmpValueTypeValue(ValueType, CurrentValue));
								...
								if (ValueType != EVoxelValueType::UseValue)
								{
									Stack.Add(FTmpValueTypeValue(EVoxelValueType::UseValue, WorldGenerator->GetValue(X, Y, Z)));
								}

								float LastValue = Stack.Last().Value;
								Stack.Pop(false);

								while (Stack.Num() > 0)
								{
									auto Pair = Stack.Pop(false);
									const EVoxelValueType& CurrentType = Pair.Type;
									const float& CurrentValue = Pair.Value;
									...
									case EVoxelValueType::UseValueIfSameSign:
										LastValue = FVoxelUtilities::HaveSameSign(LastValue, CurrentValue) ? CurrentValue : LastValue;
										break;
									default:
										check(false);
										break;
...
void FValueOctree::AddAsset(TSharedRef<FVoxelAssetInstance> Asset)
{
	if (Asset->GetWorldBounds().Intersect(GetBounds()))
	{
		if (IsLeaf())
		{
			if (LOD != 0 || !IsDirty())
			{
				Assets.EmplaceAt(0, Asset);
			}
			else
			{
				for (int X = 0; X < DATA_CHUNK_SIZE; X++)
				{
					for (int Y = 0; Y < DATA_CHUNK_SIZE; Y++)
					{
						...
						{
							FIntVector P = GetBounds().Min + FIntVector(X, Y, Z);

							float TmpValues[1];
							FVoxelMaterial TmpMaterials[1];
							FVoxelType VoxelTypes[1];
							Asset->GetValuesAndMaterialsAndVoxelTypes(TmpValues, TmpMaterials, VoxelTypes, P, FIntVector::ZeroValue, 1, FIntVector(1, 1, 1), FIntVector(1, 1, 1));

							int Index = IndexFromCoordinates(X, Y, Z);

							if (VoxelTypes[0].GetValueType() == EVoxelValueType::UseValue)
							{
								Values[Index] = TmpValues[0];
							}
							else
							{
								if (VoxelTypes[0].GetValueType() == EVoxelValueType::UseValueIfSameSign && FVoxelUtilities::HaveSameSign(Values[Index], TmpValues[0]))
								{
									Values[Index] = TmpValues[0];
								}
							}

							if (VoxelTypes[0].GetMaterialType() == EVoxelMaterialType::UseMaterial)
							{
								Materials[Index] = TmpMaterials[0];
							}
						}
					}
				}
		...
		else
		{
			for (auto Child : GetChilds())
			{
				Child->AddAsset(Asset);
			}
		}
	}
}

void FValueOctree::RemoveAssets()
{
	Assets.Empty();
}
...
void FValueOctree::AddChunksToDiffQueues(TArray<FVoxelValueDiff>& OutValueDiffQueue, TArray<FVoxelMaterialDiff>& OutColorDiffQueue)
{
	if (IsLeaf())
	{
		if (bIsNetworkDirty)
		{
			...
			for (int Index : DirtyValues)
			{
				...
				OutValueDiffQueue.Add(FVoxelValueDiff(Id, Index, Values[Index]));
			}
			for (int Index : DirtyMaterials)
			{
				check(0 <= Index && Index < DATA_CHUNK_TOTAL_SIZE);
				OutColorDiffQueue.Add(FVoxelMaterialDiff(Id, Index, Materials[Index]));
			}
			DirtyValues.Empty(DATA_CHUNK_TOTAL_SIZE);
			DirtyMaterials.Empty(DATA_CHUNK_TOTAL_SIZE);
		}
	}
	else
	{
		for (auto Child : GetChilds())
		{
			Child->AddChunksToDiffQueues(OutValueDiffQueue, OutColorDiffQueue);
		}
	}
}

void FValueOctree::LoadValueDiff(FVoxelValueDiff& Diff, TArray<FIntVector>& OutModifiedPositions)
{
	if (LOD == 0)
	{
		check(Diff.Id == Id);
		...
		check(0 <= Diff.Index && Diff.Index < DATA_CHUNK_TOTAL_SIZE);
		Values[Diff.Index] = Diff.Value;

		int X, Y, Z;
		CoordinatesFromIndex(Diff.Index, X, Y, Z);
		OutModifiedPositions.Add(FIntVector(X, Y, Z) + GetMinimalCornerPosition());
	}
	else
	{
		check(IsIdChild(Diff.Id));
		if (IsLeaf())
		{
			CreateChilds();
		}
		for (auto Child : GetChilds())
		{
			if (Child->IsIdChild(Diff.Id))
			{
				Child->LoadValueDiff(Diff, OutModifiedPositions);
				break;
			}
		}
	}
...
void FValueOctree::LoadMaterialDiff(FVoxelMaterialDiff& Diff, TArray<FIntVector>& OutModifiedPositions)
{
	if (LOD == 0)
	{
		check(Diff.Id == Id);
		...
		check(0 <= Diff.Index && Diff.Index < DATA_CHUNK_TOTAL_SIZE);
		Materials[Diff.Index] = Diff.Material;
	...
	else
	{
		...
		for (auto Child : GetChilds())
		{
			if (Child->IsIdChild(Diff.Id))
			{
				Child->LoadMaterialDiff(Diff, OutModifiedPositions);
				break;
			}
		}
	}
...
void FValueOctree::CreateChilds()
{
	...
	for (auto Child : GetChilds())
	{
		...
		{
			if (Child->GetBounds().Intersect(Asset->GetWorldBounds()))
			{
				Child->Assets.Add(Asset);
			}
		}
	}
	Assets.Reset();
